...
public class ISERandomSpreadWithChance extends StructurePlacement {
    public static final Codec<ISERandomSpreadWithChance> CODEC =
            RecordCodecBuilder.create(instance ->
            placementCodec(instance).and(instance.group(
                    RandomSpreadParams.CODEC.get().fieldOf("random_spread").forGetter(ISERandomSpreadWithChance::randomSpreadParams),
                    ChanceParams.CODEC.get().fieldOf("chance_data").forGetter(ISERandomSpreadWithChance::chanceParams)
            )).apply(instance, ISERandomSpreadWithChance::new));

    RandomSpreadParams randomSpreadParams;

    ChanceParams chanceParams;

    public ISERandomSpreadWithChance(Vec3i locateOffset, StructurePlacement.FrequencyReductionMethod frequencyReductionMethod, float frequency, int salt, Optional<ExclusionZone> exclusionZone, RandomSpreadParams randomSpreadParams, ChanceParams chanceParams) {
