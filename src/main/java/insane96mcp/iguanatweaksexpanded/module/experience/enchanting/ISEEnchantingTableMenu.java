...
public class ISEEnchantingTableMenu extends AbstractContainerMenu {
    ...
    protected ISEEnchantingTableMenu(int pContainerId, Inventory pPlayerInventory, Container pContainer, ContainerLevelAccess access) {
        ...
        this.addSlot(new Slot(pContainer, ITEM_SLOT, 20, 19) {
            ...
            public void setChanged() {
                super.setChanged();
                ISEEnchantingTableMenu.this.slotsChanged(this.container);
            }
    ...
    public void slotsChanged(Container pContainer) {
        this.access.execute((level, blockPos) -> {
            this.updateMaxCost(this.container.getItem(0), level, blockPos);
            SyncISEEnchantingTableLearnedEnchantments.sync((ServerLevel) level, (ISEEnchantingTableBlockEntity) level.getBlockEntity(blockPos));
        });
    }
    ...
    public boolean clickMenuButton(Player player, int pId) {
        ...
        this.access.execute((level, blockPos) -> {
            ...
            if (!player.getAbilities().instabuild) {
                ...
                if (!DroppedExperience.disableExperience)
