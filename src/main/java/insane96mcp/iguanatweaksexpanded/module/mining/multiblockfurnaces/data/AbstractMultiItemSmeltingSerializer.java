...
public abstract class AbstractMultiItemSmeltingSerializer implements RecipeSerializer<AbstractMultiItemSmeltingRecipe> {
    ...
    public AbstractMultiItemSmeltingRecipe fromJson(ResourceLocation pRecipeId, JsonObject pJson) {
        ...
        ISEBookCategory category = ISEBookCategory.CODEC.byName(GsonHelper.getAsString(pJson, "category", null), this.getDefaultBookCategory());
    ...
    public interface CookieBaker<T extends AbstractMultiItemSmeltingRecipe> {
        T create(ResourceLocation pId, String pGroup, ISEBookCategory pCategory, NonNullList<Ingredient> ingredients, ItemStack pResult, float outputIncrease, float pExperience, int pCookingTime, AbstractMultiItemSmeltingRecipe.Recycle recycle);
    }
